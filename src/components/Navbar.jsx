  ...
  return (
    ...
    className="flex justify-between items-center px-4 sm:px-12 lg:px-24 py-4 sticky top-0 z-20 backdrop-blur-xl font-medium bg-white/50 dark:bg-gray-900/70">
        ...
        <div className={`text-gray-700 dark:text-white sm:text-sm ${!sidebarOpen ? 'max-sm:w-0 overflow-hidden' : 'max-sm:w-60 max-sm:pl-10'} max-sm:fixed top-0 bottom-0 right-0 max-sm:min-h-screen max-sm:h-full max-sm:flex-col max-sm:bg-primary max-sm:text-white max-sm:pt-20 flex sm:items-center gap-5 transition-all`}>
            ...
            <img src={assets.close_icon} alt="Nav close icon" className='w-5 absolute right-4 top-4 sm:hidden' onClick={()=>setSidebarOpen(false)} />
        ...
        <div className='flex items-center gap-2 sm:gap-4'>
          ...
          <img src={theme === 'dark' ? assets.menu_icon_dark : assets.menu_icon} alt="Nav Icon" onClick={()=> setSidebarOpen(true)} className='w-8 sm:hidden' />
